...
import type { SpaceVO, SpaceCreateRequest } from '@/types/api'
...
const createForm = ref<SpaceCreateRequest>({
  spaceName: '',
  spaceType: 1, // 默认创建团队空间
  spaceLevel: 0
})

// 删除确认
const showDeleteConfirm = ref(false)
const spaceToDelete = ref<SpaceVO | null>(null)
...
// 团队空间列表
const teamSpaces = computed(() => spaceStore.teamSpaces)

// 获取用户在空间中的角色
function getUserRoleInSpace(space: SpaceVO): string {
  // 如果是空间创建者，返回 admin
  if (space.userId === currentUserId.value) {
    return 'admin'
  }
  // 否则从 permissionList 推断角色
  if (space.permissionList?.includes('spaceUser:manage')) {
    return 'admin'
  } else if (space.permissionList?.includes('picture:upload')) {
    return 'editor'
  }
  return 'viewer'
}
...
// 检查是否是空间所有者
function isOwner(space: SpaceVO): boolean {
  return space.userId === currentUserId.value
}
 ...
 * 确认删除空间
 */
function confirmDeleteSpace(space: SpaceVO) {
...
async function handleDeleteSpace() {
  ...
  deleting.value = true
  const success = await spaceStore.deleteSpaceById(spaceToDelete.value.id)
  ...
  <div class="space-management-page">
    ...
    <header class="page-header">
      <div class="header-content">
        ...
        <div class="header-actions">
          <button class="btn btn-ghost" @click="goToMySpace">
            <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <main class="main-content">
      <div class="content-header">
        <div class="header-title">
          <h2>团队空间</h2>
          <p class="subtitle">管理您创建或加入的团队协作空间</p>
        </div>
      ...
      <!-- 空状态 -->
      <div v-else-if="teamSpaces.length === 0" class="empty-state">
        ...
        <h3>还没有团队空间</h3>
        <p>创建团队空间与成员协作共享图片</p>
        <button class="btn btn-primary btn-large" @click="openCreateModal">创建团队空间</button>
      </div>
      ...
      <div v-else class="space-grid">
        ...
        >
          <div class="space-card-header">
            ...
            <div class="space-info">
              ...
              <div class="space-badges">
                ...
                <span class="user-role-badge" :class="getRoleClass(getUserRoleInSpace(space))">
                  {{ getRoleDisplay(getUserRoleInSpace(space)) }}
                </span>
              </div>
            </div>
            <button
          ...
          <div class="space-stats">
            ...
            <div class="stat-item">
              <span class="stat-value">{{ formatFileSize(space.spaceMaxSize) }}</span>
          ...
          <div class="space-footer">
            <div class="creator-info">
              ...
              <span class="creator-name">{{ space.userVO?.userName || '未知' }}</span>
            </div>
            <button class="btn-enter" @click.stop="enterSpace(space.id)">
              进入空间
            </button>
          </div>
        </div>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <div class="radio-group">
              <label
                v-for="option in [
                  { value: 0, label: '普通版', description: '100MB / 100张' },
    ...
    <div v-if="showDeleteConfirm" class="modal-overlay" @click.self="showDeleteConfirm = false">
      <div class="modal-content modal-small">
        ...
        <div class="modal-body">
          <p>确定要删除团队空间 "{{ spaceToDelete?.spaceName }}" 吗？</p>
          <p class="warning-text">此操作将删除空间内的所有图片，且无法撤销。</p>
        </div>
